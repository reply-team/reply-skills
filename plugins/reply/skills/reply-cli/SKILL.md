...
metadata:
  ...
  api: [user-account]
  relations:
    recommends: [reply-api, auth-and-keys]
---
...
- `TEAM_REQUIRED` / `TEAM_NOT_ACCESSIBLE` / `USER_REQUIRED` / `USER_NOT_FOUND` → see
  `auth-and-keys` skill; the CLI's stderr guidance shows the exact fix.
...
- `auth-and-keys` — key types, scopes and team/org resolution in depth.
