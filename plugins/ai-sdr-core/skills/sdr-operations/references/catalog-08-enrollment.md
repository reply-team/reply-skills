...
| `enrollment.pause` † | "Hold this person here" — with a reason and an optional dated automatic resume. | `control` | `reversible` | `auto` | the current state per person; only an active participation can be held, so per-item refusals are real answers rather than errors. | `none` | `required` | `required` | `G1.` |
