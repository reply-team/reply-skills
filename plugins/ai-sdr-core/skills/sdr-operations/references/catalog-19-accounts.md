...
| `account.update` | "Change the company's state — in play, enrolled, nurture, blocked, customer, partner — or its tier, with a reason." | `control` | `reversible` | `auto` | the current state of each account | `none` | `required` | `required` | — |
