...
| `candidate.promote`<br>*questions:* 19 | "Buy this person's details and make them ours" | `control` | `compensatable` | `confirm_once` | the per-item promotion ledger under this key: created / matched / ambiguous / withheld / failed, with the spend per item. | `metered`<br>*basis:* per person revealed, plus the enrichment allowance per field where fields are revealed one at a time<br>*meter:* `reveal` | `required` | `required` | `B1`, `B3`, `B6`, `B7`, `C2`, `C3` |
