...
|---|---|---|---|
| A request to stop | **Not a classification at all.** `optout.record` → `suppression.add(legal_obligation)` → `outreach.hold` → `scheduled_message.cancel` → `enrollment.stop(opted_out)`. **Never reply** | `auto` **protective** throughout | A2, H4, L6 |
