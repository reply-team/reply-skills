...
| `scheduled_message.cancel` | "Do not send that after all" | `control` | `compensatable` | `auto` | the pending list — an already-cancelled item is a success, not a failure. | `none` | `required` | `required` | `reports what it could not reach`, `L6.` |
