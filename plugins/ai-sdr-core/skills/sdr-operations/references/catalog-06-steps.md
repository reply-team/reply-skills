...
| `template.update` | "Change the copy" — always produces a new draft version, never mutates a published one. | `control` | `reversible` | `auto` | the template's version list for a draft created under this key | `none` | `required` | `required` | `E13.` |
...
| `template.publish` | "Make this version sendable" — a published version is immutable. | `control` | `compensatable` | `confirm_once` | the template's published version | `none` | `required` | `required` | `E13.`, `L3.` |
