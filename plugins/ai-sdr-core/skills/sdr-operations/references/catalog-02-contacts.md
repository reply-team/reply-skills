...
| `provenance.set` | "Record where this person's data came from, and under what terms" | `control` | `reversible` | `auto` | the provenance currently recorded, with its own recorded time. | `none` | `required` | `required` | `A16.`, `provenance`, `A18.`, `B6.`, `M6.` |
...
| `cooldown.set` | "Come back to this person in Q4, not before" | `control` | `reversible` | `auto` | the cooldown in force, its scope and its date | `none` | `required` | `required` | — |
