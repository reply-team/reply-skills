...
| `meeting.cancel` | "Call it off" — a booked meeting, or an outstanding proposal whose offered times we are withdrawing | `act` | `compensatable` | `confirm_each`<br>*departs:* raised: the attendee is told. | the meeting's or proposal's state; an already-cancelled one is a success on retry. | `none` | `required` | `required` | `I5` |
