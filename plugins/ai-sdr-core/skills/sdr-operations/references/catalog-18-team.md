...
| `workload_policy.define` | "State how much one person may be holding at once" — open tasks, live enrolments, touches assigned per day — and whether a breach defers the work or refuses it. | `control` | `reversible` | `auto` | the policy in force for this scope | `none` | `none` | `required` | `H11.` |
...
| `goal.define` | "Set the target" — the number, the unit this account actually pays on, the period, and the ramp for anyone not yet at full load. | `control` | `reversible` | `auto` | the goal in force for this subject and period | `none` | `none` | `required` | — |
