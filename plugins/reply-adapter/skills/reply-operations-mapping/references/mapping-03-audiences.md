...
| `contactability_policy.define` | `control` | `confirm_once` | `absent` | — | — | *because:* not evidenced — the blacklist rules are individual exclusion entries for domains, addresses and exceptions, not a policy answering send, hold or never_send per verdict and per flag. Reporting them as one would leave every verdict the rules do not mention silently defaulting to send, which is exactly the failure the policy exists to prevent. |
