...
| [inbox-triage](plugins/ai-sdr-core/skills/inbox-triage/SKILL.md) | Work the inbound replies: surface the conversations that matter first, give the user compact thread context, draft responses together, send only what was explicitly approved, and record the outcome of each conversation. Use when the user asks about replies, inbox triage, responding to prospects, or finding the interested leads. | draft | 3.0.0 |
...
| [reply-cli](plugins/reply-adapter/skills/reply-cli/SKILL.md) | Operate Reply.io from the terminal with the reply CLI: sign in (OAuth or API key), manage profiles and team context, and call any API v3 endpoint via `reply api`. Use when executing any Reply.io operation from a shell, setting up authentication, or when another Reply skill needs commands executed. | reviewed | 1.3.0 |
